...
db.customers.find(
  [
    {
      $or: [
        {
          active: { $exists: false }, //no tenga el campo active
        },
        {
          "accounts.4": { $exists: true }, //posean más de 4 cuentas
        },
        {
          birthDate: {
            //nacieron entre Abril de 1995 y Marzo de 1997 inclusives
            $gte: new Date("1995-04-01"),
            $lte: new Date("1997-03-31"),
          },
        },
      ],
    },
  ],
  {
    ...
    name: 1, //mostrar el nombre
    email: 1,
    birthDate: 1,
    n_accounts: { $size: "$accounts" }, //cantidad de cuentas
  }
);
...
db.createView("transactionCountByCode",//nombre de la vista
 "transactions", [ //nombre de la colección, pipeline
  {
    $addFields: { //agregar un campo
      buy_transactions: { //campo buy_transactions
        $filter: { //filtrar
          input: "$transactions", //input de la colección transactions
          cond: { //condición
            $eq: ["$$this.transaction_code", "buy"], //condición de igualdad
          },
        },
      },
      sell_transactions: { //campo sell_transactions
        $filter: { //filtrar
          input: "$transactions", //input de la colección transactions
          cond: { //condición
            $eq: ["$$this.transaction_code", "sell"], //condición de igualdad
          },
        },
      },
    },
  ...
  {
    $project: { //proyectar
      _id: "$_id", //id de la transacción
      account_id: "$account_id", //id de la cuenta
      transaction_count: { $size: "$transactions" }, //cantidad de transacciones
      buy_transaction_count: { $size: "$buy_transactions" }, //cantidad de transacciones de compra
      sell_transaction_count: { $size: "$sell_transactions" }, //cantidad de transacciones de venta
    },
  },
  {
    $sort: { transaction_count: -1 }, //ordenar por cantidad de transacciones
  },
]);
